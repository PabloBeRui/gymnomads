// Wrapper: useProfilePictureUpload
// Wrapper: useProfilePictureUpload
//
// Propósito: adaptar la función de servicio uploadProfilePicture al hook unificado.
// Purpose: adapt uploadProfilePicture service to the unified hook.
...
/**
 * Nota / Note:
 * - uploadProfilePicture en services tiene firma (token, file)
 * - Aquí adaptamos para que uploadFn reciba (file, token)
 * - Hacemos un casteo al tipo esperado por useImageUpload para evitar el error de TS.
 */
export const useProfilePictureUpload = () =>
  useImageUpload(
    ...
    // QUICK FIX: casteamos la función concreta al tipo esperado por useImageUpload
    // QUICK FIX: cast the concrete function to the type expected by useImageUpload
    (async (file: File, token: string) =>
      (await uploadProfilePicture(token, file)) as UploadProfilePictureResponse) as unknown as (
      file: File,
      ...args: unknown[]
    ) => Promise<unknown>,
